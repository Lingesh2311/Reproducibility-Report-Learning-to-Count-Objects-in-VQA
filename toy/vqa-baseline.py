...
with open("print-baseline.txt", "w+") as f:
    f.write(str(device) + '\n')
...
def main_wrap(objects, **kwargs):
    global acc_plot
    model = Baseline(params['objects'])  # change to different model here
...
for config in configuration:
    ...
    for name, ran in param_ranges.items():
        for x in ran:
            ...
            time_used = (end - start)
            with open("print-baseline.txt", "a") as f:
                f.write(str(px) + '     time used: ' + str(time_used) + '\n')

with open("resultacc-baseline.txt", "wb") as fp:  #Pickling
